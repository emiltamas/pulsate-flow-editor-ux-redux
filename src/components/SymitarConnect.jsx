  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#fff', zIndex: 40, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: 62, borderBottom: '1px solid #edf1f6', display: 'flex', alignItems: 'center', padding: '0 24px', gap: 14, flex: 'none' }}>
        ...
        <h1 style={{ margin: 0, fontSize: 19, fontWeight: 600, color: '#2e3d66', letterSpacing: '-.3px' }}>Connect Symitar — Standard Batch Extract v1.3</h1>
        <div style={{ marginLeft: 24, display: 'flex', alignItems: 'center', gap: 8 }}>
          {STEPS.map((label, i) => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: i === step ? '#2e3d66' : i < step ? '#1f6f4a' : '#b1bccb' }}>
                <span style={{ width: 18, height: 18, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, background: i < step ? '#e2f4ea' : i === step ? '#2e3d66' : '#e9ecf7', color: i === step ? '#fff' : '#8a95a6' }}>
        ...
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          {step === 0 && (
            <>
              ...
              <p style={{ margin: '6px 0 14px', fontSize: 14, fontWeight: 600, color: '#5a6b85', lineHeight: 1.55, maxWidth: 640 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                {RECORD_TYPES.map((t) => (
                  <div key={t} style={{ border: '1px solid #e7edf5', background: '#f7fafd', borderRadius: 4, padding: '12px 14px' }}>
                    <div style={{ fontFamily: mono, fontSize: 13.5, fontWeight: 500, color: '#2e3d66' }}>VIP.{t}.EXTRACT</div>
                    <div style={{ marginTop: 2, fontSize: 12, fontWeight: 600, color: '#8a95a6' }}>PowerOn specfile + job file</div>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 12, background: '#f7fafd', border: '1px solid #cfe1f6', borderRadius: 4, padding: '12px 16px' }}>
                <span style={{ fontFamily: mono, fontSize: 13, fontWeight: 500, color: '#2e3d66' }}>sftp://pulsate/…/inbound</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#8a95a6' }}>Nightly · 04:00 local · pipe-delimited, one row per record</span>
              </div>
            </>
          )}
          ...
          {step === 1 && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div style={{ border: '1px solid #e2e8f1', background: '#fff', borderRadius: 4, padding: '13px 15px' }}>
                  <div style={{ fontFamily: mono, fontSize: 13.5, fontWeight: 500, color: '#2e3d66' }}>VIP.LOAN</div>
                  <div style={{ fontSize: 12.5, fontWeight: 600, color: '#8a95a6', margin: '2px 0 8px' }}>{fmt(s.loanCols)} columns → {LOAN_BOUND.length} bound</div>
                  {LOAN_BOUND.map((f) => (
                    <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, fontWeight: 500, color: '#1b3a63', padding: '2px 0' }}>
                ...
                <div>
                  <div style={{ border: '1px solid #e2e8f1', background: '#fff', borderRadius: 4, padding: '13px 15px' }}>
                    <div style={{ fontFamily: mono, fontSize: 13.5, fontWeight: 500, color: '#2e3d66' }}>VIP.NAME</div>
                    <div style={{ fontSize: 12.5, fontWeight: 600, color: '#8a95a6', margin: '2px 0 8px' }}>{fmt(s.nameCols)} columns → {NAME_BOUND.length} bound</div>
                    {NAME_BOUND.map((f) => (
                      <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, fontWeight: 500, color: '#1b3a63', padding: '2px 0' }}>
                        <CheckIcon size={10} stroke="#1f6f4a" /> {f}
                      </div>
                  ...
                  <div style={{ marginTop: 10, border: '1px solid #f0d9d9', background: '#fff8f8', borderRadius: 4, padding: '13px 15px' }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#a33c3c', textTransform: 'uppercase', letterSpacing: '.5px' }}>Excluded — never stored</div>
                    <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {PII_EXCLUDED.map((f) => (
                        <span key={f} style={{ fontSize: 12.5, fontWeight: 500, color: '#a33c3c', background: '#fbe3e3', padding: '3px 9px', borderRadius: 4 }}>{f}</span>
                      ))}
                    </div>
                  </div>
                </div>
          ...
          {step === 3 && (
            <>
              ...
              <div style={{ marginTop: 10, background: '#fff', border: '1px solid #e2e8f1', borderRadius: 4, padding: 18 }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '6px 0', fontSize: 14, fontWeight: 500, color: '#1b3a63' }}>
      ...
      <div style={{ borderTop: '1px solid #edf1f6', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flex: 'none' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {step > 0 && (
            <button onClick={() => setStep(step - 1)} style={{ background: 'none', border: 'none', fontFamily: 'inherit', fontSize: 15, fontWeight: 500, color: '#5a6b85', cursor: 'pointer' }}>
              Back
            </button>
          )}
          <button
            ...
            style={{
              ...
              fontSize: 15, fontWeight: 600, color: '#fff', cursor: 'pointer', boxShadow: '0 2px 8px rgba(47,90,160,.3)', whiteSpace: 'nowrap',
            }}
          >
...
function Stat({ big, label, good, warn }) {
  return (
    <div style={{ background: '#fff', border: `1px solid ${good ? '#dcefe3' : '#f4e3c0'}`, borderRadius: 4, padding: '13px 15px' }}>
      <div style={{ fontSize: 22, fontWeight: 600, color: good ? '#1f6f4a' : '#8a6d2e', letterSpacing: '-.4px' }}>{big}</div>
      <div style={{ marginTop: 2, fontSize: 12.5, fontWeight: 600, color: '#5a6b85', lineHeight: 1.4 }}>{label}</div>
    </div>
  )
}
