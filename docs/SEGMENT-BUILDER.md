...
relational entities, typed fields, a data dictionary, multi-instance records per member.
...
get FI-chosen labels in the data dictionary. The prototype runs this end-to-end against a real
...
| **Growth path** | Every campaign mints a new menu item forever | Every new feed becomes rows in existing (or new) entities; the dictionary organizes instead of accumulating; mapped **semantic roles generate playbooks** ("Due Date reminders" exists because a recurring_date role exists) | Rows-never-schema ingestion; role-driven templates |
...
- **5 unlabeled codes** (6 records) — still targetable by raw code, flagged in the dictionary's attention queue
